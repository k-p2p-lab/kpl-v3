...
공통 수명은 Pareto `xm: 120s`, `alpha: 2.5`로 평균 **200초**입니다. 순차 producer 세 개가 평균 `0.3s` 지수분포 간격을 누적한 예약 시각에 따라 실행되어 이론상 전체 속도는 **초당 약 10개**입니다. 요청 처리 시간은 간격 안에 포함되며, 용량 대기로 늦어진 작업은 누락 없이 순서대로 따라잡습니다. 이론적 정상 상태는 **그룹당 약 667개, 전체 worker 약 2,000개 + boot 10개**입니다. 동시 실행 상한이나 실측 용량 보장이 아닙니다. 수명은 컨테이너 생성 성공부터 시작하여 startup 시간도 포함합니다.
