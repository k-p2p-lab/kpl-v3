...
| `POST` | `/api/v1/result-batches/{batchId}/retry` | Clean up previous Peers, then restart saved unfinished iterations from their first phase. Keeps completed iterations, batch ID, iteration numbers and seeds; uses fresh run IDs. Includes failed/interrupted/canceled runs, single runs and last-iteration failures. Returns `202` with the first queued run, `409` if busy/ineligible, `404` if absent. Uses the login session and `X-KPL-Request: dashboard`. |
...
`POST /api/v1/result-batches/{batchId}/retry` manually restarts saved unfinished iterations in iteration order. For example, if run 4 of 10 failed, completed runs 1–3 stay unchanged and runs 4–10 restart from their first phase. Later completed iterations are also preserved. Failed, interrupted (including never-started records after a restart), and canceled records are eligible. Each new attempt keeps its scenario and seed but receives a fresh ID; `previousRunIds` in experiment/result metadata lists its earlier attempts. Those logs remain downloadable under **Previous attempts**; only the latest saved attempt of each iteration contributes to batch progress and the mean. The original `/resume` endpoint still skips attempted runs and only requeues the never-started remainder.
...
During `phase: saving`, an individual or batch job remains `running`; status reads, experiment event ingestion and stop requests can continue while the result files are saved. Before publishing completion, the server rechecks that the attempt is current and uncanceled and, for an individual analysis, that its source has not been deleted.
...
Status identifies the batch with `batchId` (`runId` is empty) and adds selected `runIds`, `completedRuns`, `totalRuns`, `expectedRuns`, and a `membership` signature. `progress` weights each run equally; byte fields describe the current run. `snapshotAt` is the most recently captured run boundary; `runs[].asOf` records each source boundary in the artifact. Membership/state changes make status GET expose the previous completed cache as `idle`, and a new POST recomputes it. Log-only changes require `?refresh=1`. If batch membership changes during calculation or saving, the job fails instead of publishing a completed result with the old selection; retry to capture the current members.
...
Periodic Agent heartbeats use `partial: true` and split Peer records into JSON bodies of at most 10 MiB. If one Peer record alone exceeds the limit, the Agent reports an error naming that node and leaves the record unacknowledged. They include active/failed Peers and successfully stopped Peers awaiting Controller acknowledgement. Acknowledged successful exits are omitted from later periodic heartbeats and replayed after re-registration. Omission from a partial report does not stop a node or free its capacity. `GET /api/v1/status` and heartbeats with `partial` absent or false retain the full-inventory contract, including compact successful-exit records. The Agent keeps that full inventory in memory and exposes it on explicit status requests.
...
During orderly Agent shutdown or `DELETE /api/v1/nodes`, telemetry drain waits for Controller acknowledgement of both queued events and any batch already in flight, including final Peer events. A failed batch retains its order for retry; drain reports failure if acknowledgement cannot complete within its time limit.
...
Peer `/publish` has a 10-second publication timeout, including waiting behind another local publication. Request cancellation or deadline expiry releases queued requests without waiting for the current publisher and returns `504` while waiting for the gate/readiness. Queued canceled requests do not publish or emit a successful `publish` event; publication timestamps still start after the gate and readiness wait.
...
All dashboard/API reads, writes, SSE, scenario validation, analysis and downloads require a session. Unauthenticated API requests return JSON `401`; browser GET/HEAD navigation that requests HTML receives a styled **Login required** page with the same `401` status. The page is also available at `/login-required` and links to the login form and dashboard. Dashboard API/SSE session expiry opens this page once and stops reconnecting; explicit logout returns directly to `/login`. Opening the dashboard without a session still redirects to `/login`. Session-authenticated mutations also require `X-KPL-Request: dashboard` and reject cross-origin requests. The UI adds this header automatically; it never asks for another token and stores neither the password nor the session cookie in localStorage. Login accepts only JSON `{user, password}`; wrong credentials return `401`, and ten failed attempts from one connection IP within five minutes cause `429`. Sessions and failure records are bounded to 1,024 entries each. Login response writes do not hold the session lock, so a stalled login client does not block other session checks or logout.

Only the login/error-page assets, health/clock endpoint, `/metrics`, and the two Prometheus target-discovery endpoints remain public. Internal Agent registration/heartbeats/events and Peer bootstrap/discovery accept an automatically derived, domain-separated service key. It is generated from the account configuration and passed to Peers without exposing the login password to them. This key cannot access dashboard result APIs or start experiments. `KPL_API_TOKEN` and the `--token` CLI flag are retired; direct service clients must be updated together. Changing credentials requires redeploying Controller and Agents, with no active experiments or retained Peers using the old key. Grafana keeps its separate login.
