...
Use `sh scripts/swarm.sh remove` for full removal. It requires a clean Controller exit before stopping Agents, then verifies their clean exits before removing the stack. A Controller cleanup error stops this sequence for inspection; the helper does not proceed to the Agent stage after that failure.
